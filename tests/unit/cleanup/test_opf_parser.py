...
def test_isbn_extraction(raw: str, expected: str | None) -> None:
    opf = parse_opf_bytes(_opf(f'<dc:title>T</dc:title><dc:identifier opf:scheme="ISBN">{raw}</dc:identifier>'))
    assert opf is not None
...
def test_isbn_inferred_from_unmarked_identifier() -> None:
    ...
    opf = parse_opf_bytes(_opf('<dc:title>T</dc:title><dc:identifier id="bookid">9780698185395</dc:identifier>'))
    assert opf is not None
...
def test_publisher_sentinel_rejection(raw: str, expected: str | None) -> None:
    opf = parse_opf_bytes(_opf(f"<dc:title>T</dc:title><dc:publisher>{raw}</dc:publisher>"))
    assert opf is not None
...
def test_date_parsing(raw: str, expected: str | None) -> None:
    opf = parse_opf_bytes(_opf(f"<dc:title>T</dc:title><dc:date>{raw}</dc:date>"))
    assert opf is not None
...
def test_short_description_dropped() -> None:
    opf = parse_opf_bytes(_opf("<dc:title>T</dc:title><dc:description>short</dc:description>"))
    assert opf is not None
...
def test_malformed_xml_recovers() -> None:
    ...
    data = (
        ...
        b"<dc:title>Recovered Title"  # <-- missing </dc:title>
        b"<dc:creator>An Author</dc:creator>"
        b"</metadata></package>"
    )
